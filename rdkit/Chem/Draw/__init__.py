...
def _legacyMolToImage(
  ...
  """
    ...
    if legend:
        ...
        pos = size[0] / 2, int(0.94 * size[1]), 0  # the 0.94 is extremely empirical
...
if _sip_available():
    ...
    def MolDraw2DFromQPainter(
        qpainter, width=-1, height=-1, panelWidth=-1, panelHeight=-1
    ):
        ...
        d2d = rdMolDraw2DQt.MolDraw2DFromQPainter_(
            width, height, ptr, panelWidth, panelWidth
        )
...
def MolToImage(
  ...
  """
    ...
    return _moltoimg(
        mol,
        size,
        kwargs.get("highlightAtoms", []),
        kwargs.get("legend", ""),
        highlightBonds=kwargs.get("highlightBonds", []),
        drawOptions=options,
        kekulize=kekulize,
        wedgeBonds=wedgeBonds,
        highlightColor=kwargs.get("highlightColor", None),
    )
...
def MolToImageFile(
    mol, filename, size=(300, 300), kekulize=True, wedgeBonds=True, **kwargs
):
    """  DEPRECATED:  please use MolToFile instead

  """
    warnings.warn(
        "MolToImageFile is deprecated, please use MolToFile instead", DeprecationWarning
    )
...
def MolsToImage(mols, subImgSize=(200, 200), legends=None, **kwargs):
  ...
  """
    ...
    for i, mol in enumerate(mols):
        res.paste(
            MolToImage(mol, subImgSize, legend=legends[i], **kwargs),
            (i * subImgSize[0], 0),
        )
    return res
...
def shouldKekulize(mol, kekulize):
    if kekulize:
        return not any(
            bond.GetIsAromatic() and bond.HasQuery() for bond in mol.GetBonds()
        )
    return kekulize
    ...
    try:
        with rdBase.BlockLogs():
            mc = rdMolDraw2D.PrepareMolForDrawing(
                mol, kekulize=kekulize, wedgeBonds=wedge
            )
...
def _legacyReactionToImage(rxn, subImgSize=(200, 200), **kwargs):
    ...
    res = Image.new(
        "RGBA", (subImgSize[0] * len(mols), subImgSize[1]), (255, 255, 255, 0)
    )
    for i, mol in enumerate(mols):
        if mol is not None:
...
def ReactionToImage(
    ...
    **kwargs,
    ...
    if not useSVG and not hasattr(rdMolDraw2D, "MolDraw2DCairo"):
        ...
        width = subImgSize[0] * (
            rxn.GetNumReactantTemplates() + rxn.GetNumProductTemplates() + 1
        )
        if useSVG:
    ...
    **kwargs,
    ...
    if enlargedEnv:
        ...
        submol = Chem.MolFromSmiles(
            Chem.MolFragmentToSmiles(mol, atomsToUse=atomsToUse)
        )
