...
class TestCase(unittest.TestCase):
    ...
    def setUp(self):
        if (
            IPythonConsole is not None
            and Draw.MolsToGridImage == IPythonConsole.ShowMols
        ):
    ...
    def _testMolToImage(
        self, mol=None, kekulize=True, options=None, showImage=False, **kwargs
    ):
        mol = mol or self.mol
        img = Draw.MolToImage(
            mol, size=(300, 300), kekulize=kekulize, options=options, **kwargs
        )
        self.assertTrue(img)
    ...
    def testSpecialCases(self):
        ...
        self._testMolToImage(
            mol=Chem.MolFromSmiles("[2H][C@]([3H])(C)F"), options=options
        )
        # shared rings
        ...
        self._testMolToImage(
            mol=Chem.MolFromSmiles("c1ccccc1c1ccc(cc1)c1ccc(cc1)c1ccc(cc1)")
        )
    ...
    def testGridSVG(self):
        ...
        svg = Draw.MolsToGridImage(
            mols, legends=legends, molsPerRow=3, subImgSize=(200, 200), useSVG=True
        )
        self.assertIn("width='600px' height='800px'", svg)
        svg = Draw.MolsToGridImage(
            mols, legends=legends, molsPerRow=4, subImgSize=(200, 200), useSVG=True
        )
        self.assertIn("width='800px' height='600px'", svg)
        svg = Draw.MolsToGridImage(
            mols, legends=legends, molsPerRow=3, subImgSize=(300, 300), useSVG=True
        )
    ...
    def testGithub5863(self):
        ...
        rdMolDescriptors.GetMorganFingerprint(
            mol, radius=2, bitInfo=info, useChirality=True
        )
        bitId = 1236726849
